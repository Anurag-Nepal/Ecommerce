...
import com.anuragNepal.ecommerce.utils.CommonUtils;
...
public class HomeViewController {
	...
	public String saveUserDetails(@ModelAttribute User user, @RequestParam("file") MultipartFile file, Model model, HttpSession session) throws IOException 
	{
		...
		if(!ObjectUtils.isEmpty(saveUser)) 
		{
			if(!file.isEmpty()) 
			{
				// use external upload dir
				Path userProfilePath = Paths.get(uploadDir, "profile_img");
				Files.createDirectories(userProfilePath);
				Path path = userProfilePath.resolve(safeFileName);
